        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email,
                                                 screenName = model.ScreenName, firstName = model.FirstName,
                                                 lastName = model.LastName, PhoneNumber = model.PhoneNumber};
                var result = await UserManager.CreateAsync(user, model.Password);
                var customer = new Customer(model.FirstName, model.LastName, model.PhoneNumber, model.Email);
                try
                {
                ...
                if (result.Succeeded)
                {
                    ...
                    customer = customerManager.RetrieveCustomerByEmail(model.Email);
